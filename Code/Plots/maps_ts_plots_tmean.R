var = "Annual.tmeanF"
long.title = "average annual temperature (\u00B0F)" 
scale="inferno"
...
ts <- ggplot(df, aes(x=Year, y=(eval(parse(text=var))), group=CF, colour = CF)) +
  ...
  labs(title = paste0("Change in annual ",long.title), 
       x = "Year", y = "Average annual temperature (\u00B0F)") +
...
ggsave(paste0(var,"_ANN_ts.png"), plot = ts, width = 15, height = 5.25, scale = 1, path = plot.dir,bg="white")
