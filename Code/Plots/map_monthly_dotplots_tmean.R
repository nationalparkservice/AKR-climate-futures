...
dotplot <- ggplot(delta,aes(x=(eval(parse(text=delta.var))),y=season,fill=CF)) +
  ...
  labs(title = paste0("Change in seasonal average ",long.title), 
       x = "Change (°F)", y = "") +
...
annotate_figure(g, top = text_grob(paste0("Change in seasonal average ", long.title), 
                                      face = "bold", size = 20))
